...
while True:
    ...
    r = requests.get('localhost:5000/telemetry_data_gps')
    print(r.text)
    ...
    if (command =="activation"):
        r = requests.get('localhost:5000/activate')
    elif (command == "obtain")
        r = requests.get('localhost:5000/obtain_control')
    elif (command == "release")
        r = requests.get('localhost:5000/release_control')
    elif (command == "takeoff")
        r = requests.get('localhost:5000/takeoff')
    elif (command == "landing")
        r = requests.get('localhost:5000/landing')
    #r get status
    #do smth
    #r push status
